...
const Settings: React.FC = () => {
  ...
  const [deleteConfirm, setDeleteConfirm] = useState<{ type: 'user' | 'org' | 'template', id: string, name?: string } | null>(null);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6">
            ...
            <div className="space-y-6">
              ...
              <div className="grid gap-4">
                {(['week', 'month', 'quarter', 'year'] as BirthdayPeriod[]).map(period => (
                  <label key={period} className={`flex items-center gap-3 p-4 border rounded-lg cursor-pointer transition-colors ${birthdaySettings.period === period ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20' : 'border-slate-200 dark:border-slate-700'}`}>
                    <input 
                      type="radio" 
                      name="birthdayPeriod" 
                ...
                <label className={`flex items-center gap-3 p-4 border rounded-lg cursor-pointer transition-colors ${birthdaySettings.period === 'custom' ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20' : 'border-slate-200 dark:border-slate-700'}`}>
                  <input 
                    ...
                    onChange={() => updateBirthdaySettings({ ...birthdaySettings, period: 'custom', customRange: { start: new Date().toISOString(), end: new Date().toISOString() } })}
                    className="text-indigo-600 focus:ring-indigo-500"
                  />
                  <div>
                    ...
                    {users.map(u => (
                      <tr key={u.id}>
                        ...
                        <td className="px-4 py-3">
                          <span className="px-2 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded text-xs font-semibold">{u.role}</span>
                        </td>
                  ...
                  <li key={org} className="flex justify-between items-center p-3 border border-slate-100 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700/50">
                    ...
                    {!editingOrg && (
                      <div className="flex gap-3">
                        ...
                        <button onClick={() => setDeleteConfirm({ type: 'org', id: org })} className="text-slate-400 hover:text-red-600 dark:hover:text-red-400">Delete</button>
                      </div>
                    )}
                  </li>
                ))}
              ...
              <div className="space-y-3">
                {templates.map(t => (
                  <div key={t.id} className="p-4 border border-slate-200 dark:border-slate-700 rounded-lg relative group">
                    ...
                    <button 
                      onClick={() => setDeleteConfirm({ type: 'template', id: t.id, name: t.title })}
      ...
      <ConfirmationModal
        ...
        title={`Delete ${deleteConfirm?.type === 'user' ? 'User' : deleteConfirm?.type === 'org' ? 'Organization' : 'Template'}`}
        message={
          deleteConfirm?.type === 'user'
            ? `This will permanently remove ${deleteConfirm?.name} from the system. This cannot be undone.`
            : deleteConfirm?.type === 'org'
            ? `This will remove "${deleteConfirm?.name}" from the organization list. Members using this organization will need to be updated.`
            : `This will permanently delete the template "${templates.find(t => t.id === deleteConfirm?.id)?.title}".`
        }
        confirmButtonText={`Delete ${deleteConfirm?.type === 'user' ? 'User' : deleteConfirm?.type === 'org' ? 'Organization' : 'Template'}`}
